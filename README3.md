...
- Внутренний цикл последовательно сокращает количество сравнений: от `n − 1` до `1`.
- После нахождения минимального элемента происходит обмен с текущим элементом.
...
- Внутренний цикл на `i`-й итерации: `for (j = 0; j < n − i − 1; j++)`
...
### Характеристики выполнения
- Внешний цикл начинается со второго элемента: `for (i = 1; i < n; i++)`
...
### Характеристики выполнения
- Шаги задаются последовательностью (в методичке — `n/2, n/4, ..., 1`)
- На каждом шаге выполняется сортировка вставками по подпоследовательностям
- Внешний цикл: `for (gap = n/2; gap > 0; gap /= 2)`
- Внутренний цикл: `for (i = gap; i < n; i++)`
- Вложенный цикл сдвига: `while (j >= gap && arr[j − gap] > temp)`
...
- При неравномерном распределении может деградировать
...
- Область поиска сокращается на величину `F(m−2)`
